...
import { BadgeCheck, LogOut, Settings, BookOpen, Heart, Loader2 } from 'lucide-react'
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (searchParams.get('upgraded')) {
      alert('🎉 Welcome to Pro! Your plan has been upgraded.')
    }
    ...
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 mb-5">
        <div className="flex items-start gap-5">
          <div className="w-16 h-16 rounded-full bg-orange-100 flex items-center justify-center text-2xl font-bold text-orange-700 flex-none" style={{ fontFamily: 'Georgia, serif' }}>
            {(profile?.display_name || user?.email || '?')[0].toUpperCase()}
          </div>
          <div className="flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="text-xl font-bold text-gray-900" style={{ fontFamily: 'Georgia, serif' }}>
                {profile?.display_name || 'Home Cook'}
              </h1>
            ...
            <div className="flex gap-5 mt-3">
              {[
                ...
                { val: savedRecipes.length, label: 'Saved' },
              ].map(({ val, label }) => (
          ...
          <button onClick={signOut} className="flex items-center gap-1.5 text-sm text-gray-400 hover:text-red-500 transition-colors border border-gray-200 px-3 py-1.5 rounded-lg">
      ...
      <div className="flex gap-1 border-b border-gray-200 mb-5">
        {([
          ...
          <button key={id} onClick={() => setTab(id as any)}
            className={`flex items-center gap-1.5 px-4 py-2.5 text-sm border-b-2 transition-colors ${tab === id ? 'border-orange-500 text-orange-700 font-medium' : 'border-transparent text-gray-400 hover:text-gray-700'}`}>
            {label}
          </button>
        ))}
      </div>
      ...
      {tab === 'saved' && (
        savedRecipes.length === 0 ? (
          <div className="text-center py-12 text-gray-400">
            ...
            <p>No saved recipes yet. <Link href="/" className="text-orange-600 underline">Browse recipes</Link></p>
          </div>
        ) : (
      ...
      {tab === 'my' && (
        myRecipes.length === 0 ? (
          <div className="text-center py-12 text-gray-400">
            <div className="text-4xl mb-3">🍳</div>
            <p>You haven't added any recipes yet. <Link href="/add" className="text-orange-600 underline">Add one!</Link></p>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {myRecipes.map(r => (
              <Link key={r.id} href={`/recipes/${r.id}`} className="bg-white border border-gray-200 rounded-xl overflow-hidden hover:-translate-y-0.5 transition-transform">
                <div className="h-20 bg-orange-50 flex items-center justify-center text-4xl">{r.emoji}</div>
                <div className="p-2.5 text-sm font-medium text-gray-800 truncate">{r.title}</div>
                <div className="px-2.5 pb-2.5 text-xs text-gray-400 capitalize">{r.category} · {r.likes_count} likes</div>
              </Link>
            ))}
          </div>
        )
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 max-w-md space-y-4">
          ...
          {profile?.plan === 'free' && (
            <div className="mt-4 pt-4 border-t border-gray-100">
              <p className="text-sm text-gray-500 mb-2">Want AI photo macros and unlimited saves?</p>
              <Link href="/pricing" className="text-sm text-orange-600 font-medium hover:underline">Upgrade to Pro →</Link>
            </div>
          )}
        </div>
      )}
